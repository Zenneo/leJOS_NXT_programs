...
public class driveWithSensors {
	private static int waitBetweenCommands = 200; // Pause zwischen Kommandos in Millisekunden
	private static int distanceClose = 25; //Distanz, aber der sich der Roboter umdreht
	private static int distanceFar = 50; // Distanz, die erreicht werden muss, damit der Roboter weiterfährt
	private static int speedForward = 1000; // Geschwindigkeit beim Vorwärtsfahren
	...
	public static void main(String[] args) throws Exception {
		while (!Button.ESCAPE.isDown()) {
			...
			while (sonic1.getDistance() > distanceClose || sonic2.getDistance() > distanceClose) {
				drawLCD(1);
				goForward();
			...
			while (sonic1.getDistance() < distanceFar && sonic2.getDistance() < distanceFar) {
	...
	private static void drawLCD(int state) { // 1 = Vorwärts, 2 = Linksdrehen, 3 = Rechtsdrehen
		String current;
		if (state == 1) {
			current = "Vorwärts";
		} else if (state == 2) {
